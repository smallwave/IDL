...
PRO Test_GLCF_RadCoeff_TM, LandsatMetFile,Gains=Gains,Offsets=Offsets,$
                       SunAzimuth=SunAzimuth, SunElev=SunElev, $
                       TirGains=TirGains, TirOffsets=TirOffsets
    str = ''
    strWhole = ''
    OPENR, hFile, LandsatMetFile, /GET_LUN
    READF, hFile, strWhole
    PosWhole1 = STRPOS(str, 'SUN ELEVATION =')
    PosWhole2 = STRPOS(str, 'SUN AZIMUTH =')  
    FREE_LUN, hFile
    
    OPENR, hFile2, LandsatMetFile, /GET_LUN
    
    If PosWhole1 Eq -1 And PosWhole2 Eq -1 Then Begin
      WHILE NOT EOF(hFile2) DO BEGIN
        READF, hFile2, str
        Pos1        = STRPOS(str, 'SUN_ELEVATION=')
        Pos2        = STRPOS(str, 'SUN_AZIMUTH=')
        Pos10       = STRPOS(str, 'SUN ELEVATION =')
        Pos20       = STRPOS(str, 'SUN AZIMUTH =')
        If Pos1 Eq -1 And Pos2 Eq -1 And Pos10 Eq -1 And Pos20 Eq -1 Then Continue
        ...
        If Pos10 Ne -1 Then Begin
          Pos10        = Pos10 + 15
          SunElev    = FLOAT(STRMID(str,Pos10,3))
        EndIf
        ...
        If Pos20 Ne -1 Then Begin
          Pos20 = Pos20+13
          SunAzimuth = FLOAT(STRMID(str,Pos20,3))
          Break
        EndIf
      ENDWHILE
    EndIf Else Begin
      PosWhole1        = PosWhole1 + 15
      SunElev    = FLOAT(STRMID(str,Pos1,3))
      PosWhole2        = PosWhole2 + 13
      SunAzimuth = FLOAT(STRMID(str,Pos2,3))
    EndElse
    FREE_LUN, hFile2
...
;    Offsets = [-1.52000, -2.84000, -1.17000, -1.51000, -0.37000, 15.30300, -0.15000]
     Gains   = [0.67134, 1.32220, 1.04398, 0.87602, 0.12035, 0.06555]
     Offsets = [-1.52000, -2.84000, -1.17000, -1.51000, -0.37000, -0.15000]
     TirGains = [0.05537]
     TirOffsets = [15.30300]
;    data gain values = {
