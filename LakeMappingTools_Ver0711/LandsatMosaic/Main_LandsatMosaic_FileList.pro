...
PRO Main_LandsatMosaic_FileList, Landsat_DIR, LandatMosaicFile
   
    ; ############################################################
    TxtFileName      = 'H:\MosaicProcess\2010_mosaickedImages2.txt'  
    Landsat_DIR      = 'H:\CentralAsia2010_reproj\'
    LandatMosaicFile = 'H:\MosaicProcess\CentralAsia2010_mosaic2.dat'
    ; ############################################################
    ...
    IF FileCount LT 2 THEN RETURN
    
    ; Load the txt file
    FilePaths = STRARR(FileCount)
    OPENR, hFile, TxtFileName, /GET_LUN
    FILENAMES = ''
    i = 0
    WHILE ~ EOF(hFile) DO BEGIN
       READF, hFile, FILENAMES
       FilePaths[i] = Landsat_DIR+FILENAMES+'_Reproj.dat'
       i    = i+1
    ENDWHILE
    FileCount = i
    FREE_LUN, hFile
