...
FUNCTION GLOVIS_Landsat_Reflectance,LandsatFID, AcqDate, SensorID,BandIdx,$
                                    BANDREF=Band_REF
    ...
    IF Error_status NE 0 THEN BEGIN 
       STR_ERROR = 'GLOVIS_Landsat_Reflectance:  '+ STRING(Error_status) + ' :'$
                   + !ERROR_STATE.MSG
       PRINT, STR_ERROR
       CATCH, /CANCEL 
       RETURN, 0
    ...
    ENVI_FILE_QUERY, LandsatFID, NB = NB, DIMS = DIMS, BNAMES = BNAMES, $
                     DATA_GAINS=Gains, DATA_OFFSETS=Offsets
                     
    IF(N_ELEMENTS(Gains) EQ 0 OR N_ELEMENTS(Offsets) EQ 0) THEN RETURN, 0
    
    ; [1.4] sun elevation angle
    SunElev   = ENVI_GET_HEADER_VALUE(LandsatFID, 'Sun elevation angle', /FLOAT)
    IF(SunElev LE 0.0) THEN RETURN, 0
    ...
    ;**************************************************************************     
    ; Read the bands
    Band_DN  = ENVI_GET_DATA(FID=LandsatFID, DIMS=DIMS, POS=[BandIdx-1])            
    Band_MASK= Band_DN GT 0
    ; [2.1] DN to Radiance 
    Band_RAD = FLOAT(Band_DN*Gains[BandIdx-1]+Offsets[BandIdx-1])
    Band_RAD = Band_RAD * TEMPORARY(Band_MASK)
    ...
    Gain_REF = (PI*d*d)/(ESun[BandIdx-1]*COS((90-SunElev)*PI/180))
    
    Band_REF = FLOAT(Band_RAD*Gain_REF)
    Band_REF = (Band_REF GT 1)*1.0 + (Band_REF LE 1 AND Band_REF GT 0)*Band_REF
