...
PRO MAPPING_PreSegmentation,WaterMask,ImageMask,RefBand4,HillShade,Slope

  COMMON SHARE
  bHaveShade   = ( N_Elements (HillShade) EQ 0 ) ? 0 : 1
;  IF(bHaveShade) THEN ShadeMask = HillShade LT SHADE_T OR Slope GT SLOPE_T
  ...
  FOR i = 0L, NoZeroCount-1 DO BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      IF(bHaveShade) THEN BEGIN
         bShade = (Mean(Slope[Index]) GT Slope_T) OR (Mean(HillShade[Index]) LT Shade_T)
         IF bShade THEN BEGIN
            WaterMask[Index] = 0
            ImageMask[Index] = 0
         ENDIF
         GOTO, NextRegion
      ...
      IF(MEAN(RefBand4[Index]) GT SNOW_REF) THEN BEGIN
         WaterMask[Index] = 0
         ImageMask[Index] = 0
         GOTO, NextRegion
      ENDIF
